...
import CU from '../../assets/img/CU_icon.svg'
...
const StoreListCU = () => {
    ...
    const fetchAllProducts = async () => {
        try {
            setLoading(true);
            const response = await axiosInstance.get('/products/convenience/cu', {
                headers: {
            ...
            if (response.status === 200) {
                setCUProducts(response.data.data); // 초기에는 모든 데이터를 필터링 상태에 설정
            }
        } catch (error) {
    ...
    const fetchRecommendedProducts = async () => {
        try {
            ...
            if (response.status === 200) {
                ...
                const filtered = response.data.data.filter(product => product.availableAt.includes('CU'));
                setCUProducts(filtered);
            }
        } catch (error) {
    ...
    const fetchAllRecipes = async () => {
        try {
            setLoading(true);
            const response = await axiosInstance.get('/recommendations/convenience/CU', {
                headers: {
                    Authorization: `Bearer ${token}`, // 토큰이 있으면 헤더에 추가
                },
            });
    ...
    const fetchCategoryData = async (category) => {
        try {
            ...
            if (response.status === 200) {
                const filtered = response.data.data.filter(product => product.availableAt.includes('CU'));
                setCUProducts(filtered); // 가져온 데이터 설정
            }
        } catch (error) {
        ...
        <div className='store_list_wrap'>
            ...
            <div className="list_div">
                {loading ? (
                    ...
                    <Product products={CUProducts} /> // Product 컴포넌트에 데이터 전달
                ) : (
